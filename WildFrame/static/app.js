...
(function () {
  ...
  async function fetchBayesianState() {
    ...
    try {
      ...
      const url = `/api/bayesian/state?threshold=${threshold}&contour=0.6`;
  ...
  async function fetchFirmsData() {
    ...
    try {
      ...
      const kind = data.grids_considered === 0 ? null : (data.injected > 0 ? "active" : null);
      const emoji = data.injected > 0 ? "🛰️" : "ℹ️";
      _setSatelliteStatus(`Satellite: ${emoji} ${data.message || "no data"}`, kind);
  ...
  async function toggleFirmsPoller(active) {
    ...
    try {
      ...
      const res = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: active ? JSON.stringify({ interval_s: 600, day_range: 1, min_confidence: "nominal" }) : undefined,
      });
      const data = await res.json();
  ...
  async function cancelHistoricDemo() {
    ...
    // Reset Bayesian grid and refresh panel stats
    await fetch("/api/bayesian/reset", { method: "POST" });
    await fetchBayesianState();
