...
class FIRMSHotspot:
    ...
    def is_high_confidence(self) -> bool:
        ...
        # FIRMS confidence values:
        #   - "h" or "high" for high confidence
        #   - "n" or "nominal" for nominal confidence (includes % strings)
        #   - "l" or "low" for low confidence
        #   - "0-100" percentage values (occasionally returned)
        try:
            pct = int(self.confidence)
            return pct >= 80
        except (ValueError, TypeError):
            return self.confidence.lower() in ("high", "h")
    ...
    def is_nominal_or_higher(self) -> bool:
        ...
        try:
            ...
            return self.confidence.lower() in ("nominal", "n", "high", "h")

    @property
    def acquired_at(self) -> "datetime | None":
        """Parse acq_date ('YYYY-MM-DD') + acq_time ('HHMM', UTC) into an
        aware datetime. Returns None if either field is missing/malformed."""
        try:
            hhmm = str(self.acq_time).zfill(4)
            dt = datetime.strptime(f"{self.acq_date} {hhmm}", "%Y-%m-%d %H%M")
...
def fetch_fire_data(
    ...
    try:
        ...
        # NASA often returns the real reason in the response body (e.g.
        # "Invalid MAP_KEY.", "Rate limit exceeded"). Surface it instead of
        # hiding it behind a generic status line.
        ...
        raise ConnectionError(
            f"FIRMS API HTTP {exc.code}: {exc.reason}"
            + (f" — {detail}" if detail else "")
        ) from exc
